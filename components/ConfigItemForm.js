...
import { UrlRegexp } from 'lib/regexp'
...
export default function modal(props) {
  ...
  return (
    <Form
      className="ml-2 mr-3 w-full max-w-screen-md"
      {...layout}
      name="basic"
      initialValues={
        props.config ?? {
      ...
      colon={false}
    >
      <Item hidden name="_id"></Item>
      <Item hidden name="newTabIndex"></Item>
      ...
      <Item
        ...
        name="url"
        required
        tooltip="Replace search key with %s"
        rules={[
          {
            required: true,
      ...
      >
        <Input placeholder="e.g. https://search.jerrykjia.com/search?q=%s" />
      </Item>
      <Item label="Platform">
        <Item name={['platform', 'mobile']} valuePropName="checked" noStyle>
          <Checkbox>Mobile</Checkbox>
        </Item>
        <Item name={['platform', 'desktop']} valuePropName="checked" noStyle>
          <Checkbox>Desktop</Checkbox>
        </Item>
      </Item>
      <Item
        label="Embeddable"
        tooltip="Whether this site can be embedded in an iframe"
        name="embeddable"
        valuePropName="checked"
      >
        <Switch />
      </Item>
      <Item label="Priority" name="priority" tooltip="Affects the order of engines">
        <InputNumber min={0} max={20} />
      </Item>
      <Item
        label="Unavailable in"
        name="blockedRegions"
        tooltip="Regions that has blocked this site"
        rules={[{ type: 'array' }]}
      >
        <Select
          mode="multiple"
          placeholder="2 letter ISO-3166-1 country codes (e.g. US)"
          options={countries}
        />
      </Item>
      <Item
        label="Locale"
        name="locale"
        tooltip="Default language of this site. Leave empty for auto-detection"
      >
        <Select placeholder="BCP-47 language tag (e.g. en-US)" allowClear options={locales} />
      </Item>
      <Item label="Slug" name="slug" tooltip="Used to fetch styles from simple-icon">
        <Input placeholder="Enter a name" />
      </Item>

      <Item name={['style', 'hex']} label="Color">
        <Input placeholder="HEX value (e.g. #FFFFFF)" />
      </Item>
      <Item name={['style', 'svg']} label="Icon">
        <Input placeholder="SVG definition" />
      </Item>
      {/*  <Item name={['style', 'hex']} label="Color" valuePropName="color">
        <SketchPicker />
      </Item> */}
      <Item>
        <div className="space-x-4 flex justify-between" style={{ marginLeft: '120px' }}>
