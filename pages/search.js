...
import { LinkOutlined, LockOutlined, SettingOutlined } from '@ant-design/icons'
import { Dropdown, Menu, Switch, Tabs, Tooltip } from 'antd'
  ...
  useEffect(() => {
    function onKeyDown(e) {
      ...
      switch (key) {
        ...
        case 'Enter': {
          if (!editMode) handleSetSearch(inputKey)
          break
        }
        // Focus search bar on /
        case '/': {
          if (document.activeElement !== landingSearchBarRef.current && !editMode) {
            e.preventDefault()
            landingSearchBarRef?.current?.focus?.()
          }
          break
  ...
  return (
    <div className="app-container flex flex-col h-screen w-screen">
      ...
      <div className="search-header-container h-8 flex w-screen mt-2 mb-1 justify-between items-center flex-nowrap text-center flex-none head-container bg-white dark:bg-gray-900 z-10">
        ...
        <div id="search-header-right" className="flex flex-0 flex-center">
          {/* <Button
          type="primary"
          icon={<SettingFilled />}
      ...
      <div className="search-body-container flex-1 flex items-stretch">
        {editMode ? (
          <ConfigEditor config={config} setConfig={setConfig} username={props.username} />
        ) : (
          <Tabs
